...
class PeriodHLStrategyTrend(AbsStrategy):
    ...
    def in_signal(self) -> StrategySuggestion | None:
        ...
        if (
            ...
            if (
                ...
                self.fixed_stop_loss = self._donchian_50 - self._sd
    ...
    def out_signal(self) -> StrategySuggestion | None:
        ...
        if direction == 1:
            if (
                    self._ma_s < self._donchian_50 - self._sd
                    or
                    (
                            self.fixed_take_profit < self._ma_s
                    )
            ):
